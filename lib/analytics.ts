...
export function buildAttentionReason(logs: LessonLog[]): string {
  const recent = sortLogs(logs).slice(0, 6);
  const parts: string[] = [];
  const missing = recent.filter((l) => l.homework_status === 'missing').length;
  const partial = recent.filter((l) => l.homework_status === 'partial').length;
  const trend = calculateScoreTrend(recent);
  const tags = getRepeatedTags(recent, 2);

  if (missing > 0) parts.push(`최근 숙제 미제출 ${missing}회`);
  if (partial > 0) parts.push(`부분 제출 ${partial}회`);
  if (trend.direction === 'down' && trend.delta != null)
    parts.push(`테스트 점수 ${trend.delta}점 하락 추세`);
  if (tags.length > 0) parts.push(`반복 태그: ${tags.join(', ')}`);

  return parts.length > 0 ? parts.join(' · ') : '최근 기록을 확인해 주세요.';
}
...
export function getAttentionStudents(
  ...
  for (const student of students) {
    ...
    const status = deriveStudentStatus(logs);
    ...
    result.push({
      ...
      urgency: status === 'consultation' ? 'high' : 'medium',
    });
  }

  return result.sort((a, b) => {
    const order = { consultation: 0, attention: 1, stable: 2 };
