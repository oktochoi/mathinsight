...
**EduFlow**는 **재등록을 지키는 AI 상담 인프라**입니다.  
학원의 하루는 **Lesson(오늘 수업)** 중심으로 돌아가고, 출결·숙제·성적 입력 → 위험 신호 → 상담 → 학부모 전달 → 재등록 방어까지 한 루프로 이어집니다.

> 제품 비전: [docs/EDUFLOW.md](./docs/EDUFLOW.md)  
> 운영 시스템화·마이그레이션: [docs/OPERATIONS_SYSTEMIZATION.md](./docs/OPERATIONS_SYSTEMIZATION.md)  
...
1. **Dashboard** — 상담 필요·재등록 위험·미마감 수업·학부모 문의 확인  
2. **오늘 수업** (`/lesson-logs`) — 반별 출결·숙제·점수·메모 입력 후 **수업 마감**  
3. **상담센터** — AI 준비 → 상담 → 학부모 전달  
4. **학생 상세** — 위험 신호·재등록 상태·보호자 정보 확인

### 강사

1. **Dashboard (강사 업무판)** — 오늘 **내 수업**·마감 필요·담당 학생 상담 대상  
2. **오늘 수업** — 담당 반만 입력·마감  
3. 출결·숙제·성적 메뉴는 **예외/상세 관리**용

### ERP 마이그레이션 적용 순서

`023` → `024` → `025` → `026` → `027` (적용 완료 시 아래 검증 실행)

```bash
# Supabase SQL Editor에서 scripts/verify-erp-migrations.sql 실행
```
...
| 역할 | 이메일 |
|------|--------|
| 원장 | `okto0914@gmail.com` |
| 학부모 | `okto0915@gmail.com` |
| 학생 | `okto0916@gmail.com` |
...
**메뉴:** `오늘 수업` (`/lesson-logs`)

Dashboard의 「오늘 수업 입력」 또는 `?class=반ID&date=YYYY-MM-DD` 딥링크로 바로 이동할 수 있습니다.
...
- 단원
- 태그·메모 (특이사항)
- **수업 마감** (scheduled → in_progress → closed)

저장 시 `lesson_logs`에 기록되며, ERP 마이그레이션 적용 시 `lessons`·`attendance_records`와 동기화됩니다.  
출결·숙제·성적 개별 메뉴는 상세·예외 관리용입니다.
