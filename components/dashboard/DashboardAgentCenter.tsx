  ...
  return (
    <div className={cn('rounded-xl border p-4', styles[tone])}>
      ...
      <p className="text-2xl font-bold text-indigo-700 tabular-nums mt-2">
        {students.length}
        <span className="text-sm font-medium text-slate-600 ml-1">명</span>
      </p>
      {students.length === 0 ? (
        <p className="text-xs text-slate-500 mt-3">해당 학생 없음</p>
      ) : (
          ...
          {students.map((s) => (
            <li key={s.id}>
              ...
              >
                ...
                <span className="text-slate-500 ml-1 text-xs">{s.grade}</span>
                <p className="text-[11px] text-slate-600 mt-0.5 line-clamp-2">{s.reason}</p>
              </Link>
            </li>
          ))}
        </ul>
...
export function DashboardAgentCenter({ insight }: { insight: DashboardAgentInsight | null }) {
  ...
  return (
    <section className="rounded-2xl border border-violet-200/80 bg-gradient-to-br from-violet-50/40 to-white p-5 sm:p-6 space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold text-violet-700">수업 기록 기반 자동 분류</p>
          <h2 className="text-lg font-bold text-slate-900 mt-0.5">조치 유형별로 나눈 목록</h2>
      ...
      {!hasAny ? (
        <p className="text-sm text-slate-600 py-6 text-center rounded-xl bg-white border border-slate-100">
